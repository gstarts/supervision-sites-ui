      ...
      <el-row :gutter="10">
        ...
        <el-col :span="6">
          <el-form-item label="场所" prop="placeId">
            <el-select v-model="form.placeId" placeholder="请选择场所" @change="getYardZoneList">
              <el-option
                v-for="dept in depts"
                :key="dept.deptId"
                :label="dept.deptName"
...
<script>
	...
	import {getStore_detail_byStoreCode} from '@/api/place/store_detail'
		...
		methods: {
			...
			getYardZoneList() {
				if (this.form.placeId !== '') {
					listZone({'placeId': this.form.placeId, 'zoneType': '1'}).then(response => {
						this.zoneList = []
