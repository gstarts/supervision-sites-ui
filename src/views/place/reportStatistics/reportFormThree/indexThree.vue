...
<!--        </div>-->
      ...
      <el-table v-loading="loading" :data="reportList" id="analyouttable" show-summary
        ...
        <af-table-column label="库位" align="center" prop="column3"/>
        <af-table-column label="入库量(KGS)" align="center" prop="column4"/>
        <af-table-column label="出库量(KGS)" align="center" prop="column5"/>
        <af-table-column label="库存(KGS)" align="center" prop="column6"/>
...
<script>
  ...
  import {statisticsThree} from "@/api/place/info";
  ...
  export default {
    ...
    methods: {
      ...
      getInfo() {
        this.loading = true
        this.reportList = []
        statisticsThree(this.queryParams).then(response => {
          this.loading = false
