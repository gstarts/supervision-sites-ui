	...
	export default {
		...
		methods: {
			...
			print() {
				...
				/**生成入库单
          打印时是出厂，只需要判断是重进空出，还是空进重出
          重进空出，是生成入库单 需要指定库位
          空进重出是生成出库单，需要回传净重值
				 **/
				if (this.PoundForm.stationViaType === '01') { //重进空出
					genStoreDoc(this.queryParams.stationId, 1, 'noticeNo', storeCode, 0)
				}
				if (this.PoundForm.stationViaType === '02') { //空进重出
					genStoreDoc(placeId, 0, 'noticeNo', 'storeCode', 12310)
				}
				//if()
