...
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['tax:outdocdtl:export']"
        >导出</el-button>
      </el-col>
    </el-row>

    <el-table v-loading="loading" :data="outdocdtlList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="出库细单ID" align="center" prop="outDtlId" />
      <el-table-column label="出库单id" align="center" prop="outDocId" />
...
export default {
  ...
  data() {
    return {
      ...
      goodsLsist: {},
      //总单ID
      outDocId: this.$route.query.docId,
       //总单状态
      outDocStutus: this.$route.query.docStatus,
            // 装货物大类字典
      classOptions: [],
      // 单位字典
      unitOptions: [],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOutdocdtl(this.queryParams).then((response) => {
        this.outdocdtlList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.outDtlId);
    ...
    handleUpdate(row) {
      this.reset();
      const outDtlId = row.outDtlId || this.ids;
      getOutdocdtl(outDtlId).then((response) => {
    ...
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.outDtlId != undefined) {
            updateOutdocdtl(this.form).then((response) => {
            ...
            addOutdocdtl(this.form).then((response) => {
              if (response.code === 200) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const outDtlIds = row.outDtlId || this.ids;
      this.$confirm(
        '是否确认删除出库明细编号为"' + outDtlIds + '"的数据项?',
        "警告",
        {
    ...
    dunitFormat(row, column) {
      return this.selectDictLabel(this.unitOptions, row.packagesUnit);
    },
    ...
    classFormat(row, column) {
      return this.selectDictLabel(this.classOptions, row.cargoCategory);
        ...
        /** 查询明细 */
    getDtlData(outDocId) {
      listIndtlbydocid(outDocId).then((response) => {
        this.outdocdtlList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
    ...
    handleSelectionChangeIn(selection) {
      this.$set(this.idsIn, "outNoticeDocId", this.outDocId);
      this.$set(
        this.idsIn,
        "goodsId",
