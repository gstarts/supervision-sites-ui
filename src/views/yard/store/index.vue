<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      ...
      <el-form-item label="集装箱(货)号" prop="businessCode">
        <el-input
          v-model="queryParams.businessCode"
          placeholder="请输入集装箱(货)号"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        storeLevel: [
          { required: true, message: "库位层级不能为空", trigger: "blur" }
        ],
        storeState: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除库位库存编号为"' + ids + '"的数据项?', "警告", {
