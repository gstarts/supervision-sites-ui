      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="托架/拖挂车类型" prop="typeCode">
            <el-input v-model="Tform.typeCode" placeholder="请输入托架/拖挂车类型" size="mini"   />
          </el-form-item>
        </el-col>
        <el-col :span="8">
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item label="集装箱(器)编号" prop="id">
            <el-input v-model="cFrom.transportEquipmentId" placeholder="请输入集装箱(器)编号" size="mini"   />
          </el-form-item>
        </el-col>
        <el-col :span="6">
        ...
        <el-col :span="6">
          <el-form-item label="集装箱(器)自重kg" prop="tareWeight">
            <el-input v-model="cFrom.tareWeight" placeholder="请输入集装箱(器)自重kg" size="mini"   />
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
<script>
import { listHead, getHead, delHead, addHead, updateHead, exportHead ,getTransport,getCoal,getTransportDetail,getCoalDetail, updateTransportInfor,updateCoalInfor} from "@/api/manifest/rmft4406/head";
...
export default {
  ...
  methods: {
    ...
    submittForm: function() {
             updateTransportInfor(this.Tform).then(response => {
              if (response.code === 200) {
                this.msgSuccess("修改成功"); 
               // this.reset();
                 this.getListsmt();
    ...
    submitcForm: function() {
            ...
            updateCoalInfor(this.cFrom).then(response => {
              if (response.code === 200) {
                this.msgSuccess("修改成功");  
                 this.getListCoal();
                 this.clearCoalBody()
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除【请填写功能名称】编号为"' + pids + '"的数据项?', "警告", {
